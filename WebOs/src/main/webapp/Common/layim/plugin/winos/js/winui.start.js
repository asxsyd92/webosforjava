...
layui.define(['jquery', 'element', 'layer', 'winui'], function (exports) {
    ...
    var Menu = function (options) {
        this.options = options || {
            url: '/Common/winos/GetAppList.json',// winui.path + 'json/desktopmenu.json',
            method: 'GET',
    ...
    Menu.prototype.render = function (callback) {
        ...
        $(this.data).each(function (index, item) {
              ...
              var id = (item.ID === '' || item.ID == undefined) ? '' : 'win-id="' + item.ID + '"',
                    url = (item.tag == '' || item.tag == undefined) ? '' : 'win-url="' + item.tag + '"',
            ...
            if (item.children) {
                ...
                $(item.children).each(function (cIndex, cItem) {
                    var cId = (cItem.ID == '' || cItem.ID == undefined) ? '' : 'win-id="' + cItem.ID + '"',
                        ...
                        cicon =' <i class="' + cItem.icon + '"></i>'; //(cItem.icon.indexOf('fa-') != -1 && cItem.icon.indexOf('.') == -1) ? '<i class="fa ' + cItem.icon + ' fa-fw"></i>' : '<img src="' + cItem.icon + '" />';;
