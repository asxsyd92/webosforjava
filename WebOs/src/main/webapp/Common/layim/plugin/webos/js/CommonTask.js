layui.config({
    ...
    table.render({
        ...
        id: tableId,
        elem: '#role',
        url: '/Tasks/WaitList?title=&type=&desc=AddTime desc',
        ...
        cols: [[
            ...
            if (listtask !== "") {
                $("#selectqs").html(listtask);
                form.render();
            } else {
                common.get("/Tasks/GetTaskType", {}, function (resp) {
                    var ks = JSON.parse(resp);
                    var hso = "<option value=''>全部</option>";
                    $.each(ks.data, function (i, item) {
                        hso += "<option value='" + item.Tab + "'>" + item.Title + "</option>";
                        console.log(hso);
                    });
                    listtask = hso;
                    $("#selectqs").html(hso);
        ...
        switch (obj.event) {
            case 'a_search':
                ...
                table.reload(tableId, {
                    url: '/Tasks/WaitList?title=' + title +"&type="+tp+"&desc=AddTime desc"
                    , where: {} //设定异步数据接口的额外参数
                });
                $("#title").val(title);
                setTimeout(function () {
  ...
  ids += item.InstanceID + ',';
    ...
    function deleteRole(ids, obj) {
        ...
        top.layer.confirm(msg, { icon: 3, title: '删除' + obj.data.Title  }, function (index) {
            ...
            //向服务端发送删除指令
            //向服务端发送删除指令
            $.post("/Tasks/DelFormData", { key: obj.data.InstanceID, table: obj.data.t_Table }, function (resp) {
